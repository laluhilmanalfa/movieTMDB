...
class DetailMovieViewModel (private val movieRepo : MovieRepository) : ViewModel()  {
        ...
        compositeDisposable.add(
            movieRepo.getDetailMovie(movie_id = movieId.toString())
                ...
                .subscribe({ it ->
                    ...
                    isRequestMovieError.value = false
                }, {
                    isLoadingMovieContent.value = false
    ...
    fun loadReviews(){
        ...
        compositeDisposable.add(
            movieRepo.getListReview(movie_id = movieId.toString(), page = 1)
                ...
                .observeOn(io.reactivex.rxjava3.android.schedulers.AndroidSchedulers.mainThread())
                .subscribe({
                        it -> listReviewModel.value = it.results
    ...
    fun reload(){
        if(isRequestMovieError.value!!)
            loadDetailMovie()
        if(isRequestReviewError.value!!)
            loadReviews()
        if(isRequestVideoError.value!!)
            loadVideo()
    }
}